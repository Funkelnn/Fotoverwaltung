...
import { store, Photo } from '../store.js';

export async function initializePhotoDetail() {
  eventBus.on('photoSelected', (photo: Photo) => {
    displayPhotoDetail(photo);
  });
}

async function displayPhotoDetail(photo: Photo) {
  const photoDetail = document.getElementById('photoDetail') as HTMLElement;
  ...
  // Close window
  const closeButton = document.getElementById('closeDetail') as HTMLButtonElement;
  ...
  // Edit Details
  const editDetailsButton = document.getElementById('editDetails') as HTMLButtonElement;
  const detailsEditDiv = document.getElementById('detailsEdit') as HTMLDivElement;
  const deleteConfirmationDiv = document.getElementById('deleteConfirmation') as HTMLDivElement;
  ...
  const saveDetailsButton = document.getElementById('saveDetails') as HTMLButtonElement;
  saveDetailsButton.addEventListener('click', async () => {
    ...
    if (newTitle !== photo.title) updates.title = newTitle;
    if (newDate !== photo.capture_date) updates.capture_date = newDate;
    if (newTime !== photo.capture_time) updates.capture_time = newTime;
  ...
  // Bild löschen
  const deletePhotoButton = document.getElementById('deletePhoto') as HTMLButtonElement;
  ...
  // Tags
  const addTagButton = document.getElementById('addTag') as HTMLButtonElement;
  const newTagInput = document.getElementById('newTagInput') as HTMLInputElement;
  const tagSuggestions = document.getElementById('tagSuggestions') as HTMLDivElement;
  ...
  const removeTagButtons = photoDetail.querySelectorAll('.remove-tag') as NodeListOf<HTMLButtonElement>;
  ...
  // Album Management
  const addAlbumButton = document.getElementById('addAlbum') as HTMLButtonElement;
  const albumDropdown = document.getElementById('albumDropdown') as HTMLSelectElement;
  ...
  const removeAlbumButtons = photoDetail.querySelectorAll('.remove-album') as NodeListOf<HTMLButtonElement>;
