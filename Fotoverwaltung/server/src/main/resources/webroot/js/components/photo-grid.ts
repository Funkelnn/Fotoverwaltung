...
export async function initializePhotoGrid() {
  ...
  eventBus.on('photoUpdated', (updatedPhoto: Photo) => {
    updatePhotoInGrid(updatedPhoto);
  });

  eventBus.on('photoDeleted', (deletedPhotoId: number) => {
...
function displayPhotos(albumId: string) {
  ...
  if (album) {
    ...
    populatePhotoGrid(photos as Photo[]);
  }
}

async function populatePhotoGrid(photos: Photo[]) {
...
function updatePhotoInGrid(updatedPhoto: Photo) {
...
function removePhotoFromGrid(deletedPhotoId: number) {
