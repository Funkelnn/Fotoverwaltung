...
import { store, Album } from '../store.js';

export function initializeEditAlbumModal() {
  const modalContainer = document.getElementById('edit-album-modal') as HTMLElement;
  const modalCloseButton = document.getElementById('modal-close-edit-album') as HTMLElement;
  const modalSaveButton = document.getElementById('modal-save-edit-album') as HTMLButtonElement;
  const albumTitleInput = document.getElementById('album-title-edit-input') as HTMLInputElement;
  const albumTagsList = document.getElementById('album-tags-list') as HTMLElement;
  const albumTagsInput = document.getElementById('album-tags-input') as HTMLInputElement;
  const addAlbumTagButton = document.getElementById('add-album-tag-button') as HTMLButtonElement;
  const deleteAlbumButton = document.getElementById('delete-album-button') as HTMLButtonElement;
  const tagSuggestionsContainer = document.getElementById('tag-suggestions') as HTMLElement;

  let currentAlbum: Album;

  eventBus.on('editAlbum', async (albumId: number) => {
    currentAlbum = store.getAlbumById(albumId) as Album;
  ...
  function displayTags(tagIds: number[]) {
  ...
  function displayTagSuggestions(query: string) {
    tagSuggestionsContainer.innerHTML = '';
    if (query) {
      // Filtere Tags, die bereits zum Album gehören, aus
      const suggestions = store.getTags().filter(tag =>
        tag.name.toLowerCase().includes(query) && !currentAlbum.tag_ids.includes(tag.tag_id)
      );
